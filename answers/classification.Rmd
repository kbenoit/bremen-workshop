...
1. Extract the predicted sentiment by predicting tmod on the dfmat_test.

```{r}
pred <- predict(tmod, newdata = dfmat_test, force = TRUE)
head(pred)
table(pred)
```
...
```{r}
table(pred, dfmat_test$sentiment)[2:1, 2:1] %>%
...
```{r}
tmod2 <- textmodel_svm(dfmat_train, y = dfmat_train$sentiment)
pred2 <- predict(tmod2, newdata = dfmat_test, force = TRUE)
table(pred2, dfmat_test$sentiment)[2:1, 2:1] %>%
    caret::confusionMatrix()

dfmat_train2 <- dfm_tfidf(dfmat_train)
dfmat_test2 <- dfm_tfidf(dfmat_test)
tmod2 <- textmodel_svm(dfmat_train2, y = dfmat_train$sentiment)
pred2 <- predict(tmod2, newdata = dfmat_test2, force = TRUE)
table(pred2, dfmat_test$sentiment)[2:1, 2:1] %>%
    caret::confusionMatrix()
